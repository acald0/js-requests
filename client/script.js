...
sayHelloButton.addEventListener("mouseover", changeToBandW)
...
const ohMy = () => {
    ...
    .then((response) => {
        for(let i = 0; i < response.data.length; i++) {
            const para = document.createElement("p")(response.data[i])
        }
    })
}

const createNewPElem = () => {
    let newP = 
}
...
const repeatMyParam = () => {
    ...
    .then((response) => {
        ...
        let repeatTextId = document.getElementById('repeat-text')
        repeatTextId.textContent = response.data
        repeatTextId.style.display = 'block'
    })
